...
<div class="container">
	<div class="newSearch" id="containerModalSearchProduct" style="display: none;">
		<table id="tableSearchHelpUser" class="table-bordered table-hover" width="100%">
	...
	$(document).ready(function() {
		$(function listProduct() {
			$(document).on("submit", "#frm_SearcHelpUser", function(event) {
				...
				var tableSearchHelpUser = $("#tableSearchHelpUser").DataTable({
					dom: "Bfrtip",
					buttons: [{
						extend: "excelHtml5",
					...
					drawCallback: () => { tableSearchHelpUser.columns.adjust(); },
					ajax: {
		...
		$(function viewUsersInfo() {
			$(document).on("click", "#viewUsersInfo", function() {
				let data = $("#tableSearchHelpUser").DataTable().row($(this).parents("tr")).data();
